...
OLD_USER=$(oc exec -n ${OLD_NS} $LEADER -- printenv | grep APP_USER | sed 's/.*=//g')
OLD_DB=$(oc exec -n ${OLD_NS} $LEADER -- printenv | grep APP_DATABASE | sed 's/.*=//g')

# Dump and copy down
oc exec -n ${OLD_NS} $LEADER -- mkdir -p /tmp/dump
oc exec -n ${OLD_NS} $LEADER -- pg_dump -U $OLD_USER -d $OLD_DB -Fc -f /tmp/dump/dump_old --no-privileges --no-tablespaces --schema=public --no-owner
...
# Tables
echo "--- old ---"
oc exec -n ${OLD_NS} $LEADER -- psql -U $OLD_USER -d $OLD_DB -c "select count (*) from pg_tables;"
echo "--- new ---"
